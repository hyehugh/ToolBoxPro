          ...
          <tbody>
            ...
            {comparison.toolboxProAdvantages.map((adv, i) => (
              <tr key={`tp-${i}`} className="border-b last:border-0">
                <td className="py-2 px-2 text-muted-foreground">{adv}</td>
        ...
        {comparison.competitors.map((c) => (
          <div key={c.name} className="p-4 rounded-lg border bg-card text-sm">
            ...
            <div className="space-y-1">
              {c.pros.map((pro, i) => (
                <p key={`pro-${i}`} className="text-green-600 dark:text-green-400 text-xs">+ {pro}</p>
              ))}
              {c.cons.map((con, i) => (
                <p key={`con-${i}`} className="text-red-500 dark:text-red-400 text-xs">- {con}</p>
              ))}
            </div>
          </div>
        ))}
